...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i <
		(int)systemInfo.dwNumberOfProcessors * 2
		; i++) {
...
unsigned __stdcall serverWorkerThread(LPVOID completionPortID) {
	...
	while (TRUE) {
		...
		if (perIoData->operation == RECEIVE) {
			...
			//handle recv data
			byteStream(perIoData, queRecvData, perHandleData->socket);
			...
			{	
				...
				recvDataProcess(recvData, perHandleData, perIoData);
			}
		}
		...
		if (perIoData->bufferLen > perIoData->sentBytes) {
			// Post another WSASend() request.
			// Since WSASend() is not guaranteed to send all of the bytes requested,
			// continue posting WSASend() calls until all received bytes are sent.
			ZeroMemory(&(perIoData->overlapped), sizeof(OVERLAPPED));
			perIoData->dataBuff.buf = perIoData->buffer + perIoData->sentBytes;
			perIoData->dataBuff.len = perIoData->bufferLen - perIoData->sentBytes;
